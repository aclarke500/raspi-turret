...
from raspi_turret.hardware.camera import Camera, create_camera
from raspi_turret.hardware.gpio import GpioBoard
from raspi_turret.hardware.servo import ServoMotor
from raspi_turret.logging.log_buffer import get_logs, install_log_capture
from raspi_turret.stream.publisher import StreamPublisher
from raspi_turret.turret.turret import Turret
...
gpio_board: GpioBoard | None = None
...
def build_hardware():
    """Composition root: wire GPIO, servos, camera, detector, turret, stream."""
    gpio = GpioBoard()
    gpio.setup()

    pan = ServoMotor(
        X_SERVO_PIN, PAN_MIN_ANGLE, PAN_MAX_ANGLE, gpio, name="pan"
    )
    tilt = None
    if Y_SERVO_ENABLED:
        tilt = ServoMotor(
            Y_SERVO_PIN, Y_MIN_ANGLE, Y_MAX_ANGLE, gpio, name="tilt"
        )
    ...
    turret_instance = Turret(
        pan=pan, tilt=tilt, detector=detector, camera=cam
    )
    stream = StreamPublisher(camera=cam, detector=detector)
    return gpio, cam, turret_instance, stream
...
def run_turret_loop(stop_event: threading.Event, turret_instance: Turret):
    ...
    try:
        while not stop_event.is_set():
            x_offset, y_offset = turret_instance.patrol(reset_home=False)
            if x_offset is not None:
                turret_instance.follow_target()
                turret_instance.hold_last_angle()
    finally:
        turret_running = False
...
async def lifespan(app: FastAPI):
    global gpio_board, camera, turret, stream_publisher, turret_thread

    gpio_board, camera, turret, stream_publisher = build_hardware()
    turret.setup()
